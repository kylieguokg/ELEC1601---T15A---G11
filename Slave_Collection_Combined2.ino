...
void loop()
{
    ...
    Serial.println("irLeft"+String(irLeft)+"irRight"+String(irRight));
    ...
    if (automaticMode == true){
        ...
        int move = automatic(irLeft, irRight);
    
//        if (count > sizeof(movesArray)){
//            int newArray[sizeof(movesArray) * 2];
//          for (int i = 0; i < sizeof(movesArray); i++){
//              newArray[i] = movesArray[i];
//          }
//          movesArray = newArray;
//        }
    ...
    while(1)
    {
        if(blueToothSerial.available())   // Check if there's any data sent from the remote Bluetooth shield
        {
            ...
            if (recvChar == 't'){ //automatic
              manualMode = false;
            }
            if (recvChar == 'z')
            {
              startToFinish(movesArray, count);
            }
        }
...
void startToFinish(int arr[], int count)
{
  for (int i = 0; i < count; i++){
    int currentMove = arr[i];
    if (currentMove == 0){ // forwards
      maneuver(200, 200, 20);
    }
    else if (currentMove == 1){ // right
      maneuver(200, -200, 20);
    }
    else if (currentMove == 2){ // left
       maneuver(-200, 200, 20);
    }
...
int automatic(int irLeft, int irRight){
    
    
    if((irLeft == 0) && (irRight == 0))        // If both sides detect
    {
      //Serial.println("does it enter");
      maneuver(200, 200, 20);                // Forward 20 milliseconds
      return 0; // Forward for 20 ms --> forward = 0
    ...
    {
      maneuver(200, -200, 20);
      return 1; // Right for 20 ms --> right = 1
      
    }
    else if(irRight == 0)                      // If only right side detects
    {
      maneuver(-200, 200, 20);   // Left for 20 ms --> left = 2
      return 2;
    }
